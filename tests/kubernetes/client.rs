...
pub async fn get_kube_client(
    ...
    let conf_yaml = fs::read_to_string(&source_dir)
        .await
        .context("Error reading k3s.yaml")?;

    let mut config = Kubeconfig::from_yaml(&conf_yaml).context("Error loading kube config")?;
    ...
    let client = Client::try_from(client_config).context("Error building client")?;
