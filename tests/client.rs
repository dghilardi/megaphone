...
impl MegaphoneRestClient {
    ...
    pub async fn create(&self, req: &ChannelCreateReqDto) -> anyhow::Result<ChannelCreateResDto> {
        ...
        let resp = client
            .post(format!("http://{}:{}/create", self.host, self.port))
            .json(req)
            .send()
            .await
    ...
    pub async fn write(
    ...
    ) -> anyhow::Result<BasicOutcomeDto> {
        let client = reqwest::Client::new();
        let resp = client
            .post(format!(
                "http://{}:{}/write/{producer_address}/{stream_id}",
                self.host, self.port
            ))
            .json(&payload)
            .send()
        ...
        if resp.status().is_success() {
            resp.json().await.context("Error parsing response")
        } else {
            let status = resp.status();
            eprintln!(
                "HTTP error {status} - {}",
