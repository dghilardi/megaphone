...
impl MegaphoneSyncService {
    pub fn new(agent_mgr: AgentsManagerService, megaphone_svc: MegaphoneService<EventDto>) -> Self {
        Self {
            agent_mgr,
            megaphone_svc,
        }
...
impl SyncService for MegaphoneSyncService {
    async fn forward_events(
        ...
        while let Some(stream_item) = stream.next().await {
            match stream_item {
                Ok(SyncRequest {
                    sync_event: Some(SyncEvent::PipeAgentStart(req)),
                }) => {
                ...
                Ok(SyncRequest {
                    sync_event: Some(SyncEvent::PipeAgentEnd(req)),
                }) => {
                ...
                Ok(SyncRequest {
                    sync_event: Some(SyncEvent::ChannelDisposed(_req)),
                }) => {}
                Ok(SyncRequest {
                    sync_event: Some(SyncEvent::ChannelCreated(req)),
                }) => {
                    let out = self
                        .megaphone_svc
                        .create_channel_with_id(&req.channel_id)
                        .await;
                    if let Err(err) = out {
                ...
                Ok(SyncRequest {
                    sync_event: Some(SyncEvent::EventReceived(req)),
                }) => {
                ...
                Ok(SyncRequest { sync_event: None }) => {
                    log::warn!("Received grpc SyncRequest without sync_event")
                }
...
fn timestamp_to_datetime(timestamp: Timestamp) -> Option<DateTime<Utc>> {
    let naive =
        NaiveDateTime::from_timestamp_opt(timestamp.seconds, cmp::max(0, timestamp.nanos) as u32)?;
...
impl TryFrom<EventReceived> for EventDto {
    ...
    fn try_from(value: EventReceived) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            body: serde_json::from_str(&value.json_payload).map_err(|err| {
                MegaphoneError::BadRequest(format!("Cannot deserialize json payload - {err}"))
            })?,
        })
    }
}
