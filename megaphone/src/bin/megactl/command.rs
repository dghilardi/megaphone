    ...
    Res: Printable,
{
    match (command().await, out_format) {
        ...
        (Err(error), OutFormat::Json) => eprintln!("{}", serde_json::to_string(&json!({ "out": "error", "message": error.to_string() })).unwrap()),
    }
}
...
where T: PrintFormat<JsonFormat> + PrintFormat<PlainFormat> {
    fn print(&self, format: OutFormat) {
        match format {
